    ...
    const fetchUserData = async () => {
      try {
        const userResponse = await fetch(
          `https://movie-radar-1.onrender.com/user/${username}`
        );
        if (!userResponse.ok) {
        ...
        const reviewsResponse = await fetch(
          `https://movie-radar-1.onrender.com/user/reviews/byusername/${username}?currentUserId=${userId}`
        );
        ...
        const playlistsResponse = await fetch(
          `https://movie-radar-1.onrender.com/user/playlists/${username}`
        );
